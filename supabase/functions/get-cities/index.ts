...
// supabase/functions/get-cities/index.ts
...
serve(async (req) => {
  console.log("get-cities function invoked.");
  ...
  try {
    ...
    console.log(`Received request for provinceId: ${provinceId}`);
    if (!provinceId) throw new Error("provinceId is a required parameter.");
    ...
    const endpoint = `${RAJAONGKIR_BASE_URL}/destination/city/${encodeURIComponent(provinceId)}`;
    ...
    console.log("Successfully fetched cities. Normalizing response.");
    const responseBody = {
      rajaongkir: {
        status: json.meta,
        results: json.data || [],
    ...
    console.error("Error in get-cities function:", err.message);
