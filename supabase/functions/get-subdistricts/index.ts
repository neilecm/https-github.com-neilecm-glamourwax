...
// supabase/functions/get-subdistricts/index.ts
...
serve(async (req) => {
  console.log("get-subdistricts function invoked.");
  ...
  try {
    ...
    console.log(`Received request for districtId: ${districtId}`);
    if (!districtId) throw new Error("districtId is a required parameter.");
    ...
    const endpoint = `${RAJAONGKIR_BASE_URL}/destination/sub-district/${encodeURIComponent(districtId)}`;
    console.log(`Fetching from endpoint: ${endpoint}`);

    const res = await fetch(endpoint, {
      method: 'GET',
      headers: {
        'Key': RAJAONGKIR_KEY, // Capital 'K' is required
    ...
    console.log("Successfully fetched subdistricts. Normalizing response.");
    const responseBody = {
      rajaongkir: {
        status: json.meta,
        results: json.data || [],
    ...
    console.error("Error in get-subdistricts function:", err.message);
